import { storeTerm, updateTerm } from "@/api/termination";
...
function ProcessModal(props: {
	...
	useEffect(() => {
		if (info.zzjzrq) info.zzjzrq = dayjs(info.zzjzrq);
		if (info.sjzxrq) info.sjzxrq = dayjs(info.sjzxrq);
		if (info.swsj) info.swsj = dayjs(info.swsj);
		if (info.zzjzrq) info.zzjzrq = dayjs(info.zzjzrq);
	}, [info]);
	...
	const getSteps = (detail: TerminationInfo) => {
