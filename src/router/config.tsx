...
function getPageItem(id: string, title: string, component?: () => JSX.Element) {
  return {
    id,
    title,
    component,
...
const pageMap = {
  ...
  // 不准出境
  noExit: getPageItem("noExit", routeNameMap.noExit, Investigatorsevaluated),
...
export const RouterData = routerItems.map((item) => {
  ...
  if (children == null) {
    return <Route key={page.id} path={page.id} element={<page.component />} />;
  } else {
    return children.map((p) => {
      const pathName = page.id + "/" + p.id;
      return <Route key={pathName} path={pathName} element={<p.component />} />;
    });
  }
});
